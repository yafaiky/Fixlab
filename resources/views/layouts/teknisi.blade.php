...
<div x-data="{ sidebarOpen: false }" class="flex h-screen">
    ...
    >
        ...
        <nav class="flex-1 p-4 space-y-2">
            <a href="{{ route('teknisi.dashboard') }}"
               ...
               {{ request()->routeIs('teknisi.dashboard') ? 'bg-blue-600' : 'hover:bg-gray-700' }}">
                Dashboard
            </a>

            <a href="{{ route('teknisi.service') }}"
               @click="sidebarOpen = false"
               class="block rounded px-3 py-2
               {{ request()->routeIs('teknisi.service') ? 'bg-blue-600' : 'hover:bg-gray-700' }}">
                Service
            </a>
        </nav>
    </aside>
    ...
    <div class="flex-1 flex flex-col">
        ...
        <header class="bg-white shadow p-4 flex justify-between items-center">
            ...
            <h1 class="text-xl font-bold">Teknisi Dashboard</h1>
